...
private const val TEST_URL = "https://www.baby-dump.nl/product/123"

class BabyDumpPriceParserTest {

    private lateinit var parser: BabyDumpPriceParser
    private lateinit var document: Document
    private lateinit var priceElement: Element

    @BeforeEach
    fun setUp() {
        parser = BabyDumpPriceParser()
    ...
    fun `should parse price successfully when price element is present`() {
        setupPriceElement()
        setupPriceValue("39,99")

        val result = parser.parse(document)

        assertSuccessfulParsing(result, "39.99")
    }
    ...
    private fun setupPriceValue(price: String) {
        every { priceElement.attr("data-price-amount") } returns price
    }

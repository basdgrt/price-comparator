...
private const val TEST_URL = "https://little-dutch.com/products/wipstoel-beige-essentials"

class LittleDutchPriceParserTest {

    private lateinit var parser: LittleDutchPriceParser
    private lateinit var document: Document
    private lateinit var priceElement: Element

    @BeforeEach
    fun setUp() {
        parser = LittleDutchPriceParser()
    ...
    fun `should parse price successfully when price element is present`() {
        setupPriceElement()
        setupPriceValue("€89,95")

        val result = parser.parse(document)

        assertSuccessfulParsing(result, "89.95")
    }

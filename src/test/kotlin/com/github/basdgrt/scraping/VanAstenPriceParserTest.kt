...
private const val TEST_URL = "https://www.vanastenbabysuperstore.nl/product/123"

class VanAstenPriceParserTest {

    private lateinit var parser: VanAstenPriceParser
    private lateinit var document: Document
    private lateinit var priceElement: Element

    @BeforeEach
    fun setUp() {
        parser = VanAstenPriceParser()
    ...
    fun `should parse price successfully when price element is present`() {
        setupPriceElement()
        setupPriceValue("179,00")

        val result = parser.parse(document)

        assertSuccessfulParsing(result, "179.00")
    }
